...
- B-2 (e2e): selecting a muscle group in the Exercise Library screen renders the filtered list with video links, reachable through the app.

**PR will contain:**
- `src/data/exercises.ts` (static exercise -> muscle -> YouTube URL data)
- `src/domain/exerciseFilter.ts` (+ its unit tests, B-1)
- `src/pages/ExerciseLibrary.tsx` (+ its test, B-2), wired into `src/App.tsx` alongside Routine Builder
